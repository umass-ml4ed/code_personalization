...
def error_label_from_row(row):
    return error_label(problem=row['prompt'], language='java', code=row['Code'])
...
def error_label(problem, language, code, model='gpt-4o', temperature=0):
    ...
    """
            ...
            system_content = """You are an expert Python programming instructor and automated code reviewer. Given a Python programming problem and a student's buggy code solution, your task is to identify all errors present in the code. There is at least one error in the code. Use concise and standardized label/taxonomy for each error. Make sure the error label is generalizable without problem specific description. Take the following error label examples as reference:
    Syntax Error (Examples): Indentation error, Unbalanced parentheses, Missing colon error
    Runtime Error (Examples): Name error, Type mismatch error, Index error
    Logical Error (Examples): Off-by-one error, Zero division error, Infinite Loop
...
def batch_error_label(df, lang='Java'):
    problems = df['prompt'].tolist()
    codes = df['Code'].tolist()

    if lang == 'Java':
        system_message = """You are an expert Java programming instructor and automated code reviewer. Given a Java programming problem and a student's buggy code solution, your task is to identify all errors present in the code. There is at least one error in the code. Use concise and standardized label/taxonomy for each error. Make sure the error label is generalizable without problem specific description. Take the following error label examples as reference:
    Syntax Error (Examples): Confusing assignment with equality, Unbalanced parentheses, Semicolon errors
    Runtime Error (Examples): Uninitialized Variables, Parameter confusion, NullPointerExceptions
    Logical Error (Examples): Off-by-one errors, Integer Division, Infinite Loops
    ...
    """
        ...
        system_message = """You are an expert Python programming instructor and automated code reviewer. Given a Python programming problem and a student's buggy code solution, your task is to identify all errors present in the code. There is at least one error in the code. Use concise and standardized label/taxonomy for each error. Make sure the error label is generalizable without problem specific description. Take the following error label examples as reference:
    Syntax Error (Examples): Indentation error, Unbalanced parentheses, Missing colon error
    Runtime Error (Examples): Name error, Type mismatch error, Index out of bound error
    Logical Error (Examples): Off-by-one error, Zero division error, Infinite Loop
    ...
    judge_res = client.get_responses(prompts, 'gpt-4o-mini', system_message, generation_kwargs, False)
    
    end_time = time.perf_counter()
    print(f'Total Error labeling time: {(end_time - start_time) / 60} mins')


    error_ls_total = []
    for res in judge_res:
        if res:
            try:
                error_dict = json.loads(res)
                error_ls = error_dict.get('errors', [])
                error_i = []
                for item in error_ls:
                    error_i.append((item['Category'], item['Label']))
                
                error_ls_total.append(error_i)
            except:
                error_ls_total.append([])
        else:
            error_ls_total.append([])
    

    df['error_labels'] = error_ls_total
    df.to_pickle('data/codeworkout_all_attempt_error.pkl')
    return df
...
def error_cluster_summarize(error_list, model='gpt-4o-mini', temperature=0, n=1):
    try:
        user_prompt = f"The error list is:\n{error_list}" + "\n\nNow follow the instructions in system message. First, examine the error list carefully to understand their shared meaning. Second, explicitly reason about the error. Third, based on the reasoning, select one error that best represents the majority error in the group."

        system_content = """You are an experienced computer science teacher. You will be provided with a list of errors from student code that refer to the same underlying errors but may vary in wording.

Your task is to:
1. Carefully examine all the errors in the list to ensure none are overlooked.
2. Reason explicitly the error that is represented by the majority of the errors in the list.
3. Based on your reasoning: select one error from the list that best represents the group — choose the one that is most clearly worded, generalizable, and inclusive of the others.

Return your output strictly in the following JSON format:
{
  "reasoning": "...",        // Exactly one sentence explaining your reasoning on the majority error
  "representative_error": "..." , // Selected error name
}
"""
#         user_prompt = f"The error list is:\n{error_list}" + "\n\nNow follow the instructions in system message. First, examine the error list carefully to understand their shared meaning. Second, explicitly reason about the error. Third, based on the reasoning, either select one error that best represents the group if they share the meaning or summarize your analysis into one clear and concise phrase that accurately captures the essence of this cluster."

#         system_content = """You are an experienced computer science teacher. You will be provided with a list of errors from student code that may vary in wording but sometimes refer to the same or related underlying errors.
...
# """
        ...
        representative = kc['representative_error']
