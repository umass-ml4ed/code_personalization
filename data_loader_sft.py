...
def read_data(file, configs):
    ...
    # clean code: remove comments and leading/trailing spaces in java code
...
def tokenize(tokenizer, completions, train=True):
    if train:
        # set different max length according to the model
        completions_tokenized = tokenizer(completions, truncation=True, padding=True, max_length=1200, return_tensors='pt')
        ...
        prompt_id_lens = torch.add(prompt_id_lens, 2) # adding 2 since delimiter token is assistant and and there is \n following it
...
def build_prompt_with_special_tokens_ability(prompt, tokenizer, ability_ls, kc_ls, language='java'):
    system_content = f"You are a student code simulator. Given a programming problem and the student's mastery levels for specific knowledge components (KCs), generate {language} code that reflects that understanding, including plausible student errors. Output only the code, with no explanations or comments."
...
def build_input_with_special_tokens_ability(prompt, code, tokenizer, ability_ls, kc_ls, language='java'):
    system_content = f"You are a student code simulator. Given a programming problem and the student's mastery levels for specific knowledge components (KCs), generate {language} code that reflects that understanding, including plausible student errors. Output only the code, with no explanations or comments."
